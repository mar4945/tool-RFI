...
def load_data():
    ...
    # Load JSON file
    with open('Resources\simulation_variables.json', 'r') as file:
        data = json.load(file)
...
def create_list():
    ...
    global error_f_list, time_list, rlp_s_list, rlp_v_list, b_list, c_list, flag_eme_list
...
def run_simulation():
    ...
    for t in range(0, int(time_simulation/ts), 1):
        ...
        s_f, v_f, a_f, j_f, u_f_control, result_f, delay_channel, z_tau_3, z_tau_2, z_tau_1, z_region, ref_tau = follower.step_follower( v_l, timestamp, message_l_channel)
