...
\newabbreviation{ddpg}{DDPG}{Deep Deterministic Gradient Policy}
...
\If{ $x_1^{\mathrm{F}}(t)\leq z_{\tau_3}^\mathrm{F}(t,\underline{x}^\mathrm{L}(t),x_1^\mathrm{F}(t),t_{\mathrm{last}})$ }
...
\ElsIf{ $z_{\tau_3}^\mathrm{F}(t,\underline{x}^\mathrm{L}(t),x_1^\mathrm{F}(t),t_{\mathrm{last}})< x_1^{\mathrm{F}}(t) \leq z_{\tau_1}^\mathrm{F}(t,\underline{x}^\mathrm{L}(t),x_1^\mathrm{F}(t),t_{\mathrm{last}})$ }
...
%  					height=3cm,
 ...
 \subsection{Operational scenarios}
  ...
  In the first scenario, referred to as \gls{os}1, we examine a situation where the leader and follower initiate a \gls{vc} operation, starting from \gls{l3}, with a separation distance of $6000\unit{\meter}$. As they proceed, the leader dynamically adjusts its velocity, introducing variability along the route. At a specific point, the leader stabilizes its speed and maintains a constant velocity. Despite these changes, the follower is able to safely close the gap and establish virtual coupling. As the scenario progresses, the leader unexpectedly initiates a braking maneuver, gradually slowing down until it comes to a complete stop. This scenario is designed to test the control system’s ability to manage dynamic speed variations, while ensuring precise coordination between the leader and follower. It specifically assesses the follower’s ability to respond effectively and safely to sudden changes in the leader’s behavior, such as abrupt braking.
  
 The final operational scenario, designated as \gls{os}2, begins with the leader and follower engaged in a \gls{vc} operation, where the leader initially maintains a steady, constant speed while the follower keeps a safe distance. In this scenario, the focus shifts to a situation where the communication between the leader and follower begins to degrade, causing increased delays in the transmission of control data. As the scenario unfolds, the communication further deteriorates until updates from the leader fail to arrive on time. This delay presents a critical challenge, testing the robustness and resilience of the follower’s control system. With real-time data no longer available, the follower must rely on the last received information to ensure safety, maintain proper spacing, and continue operations despite the communication breakdown. This scenario highlights the crucial role of fallback mechanisms and adaptive control strategies in ensuring operational stability when faced with unexpected communication failures, emphasizing the system’s ability to handle disruptions without compromising safety.
 ...
 The two operational scenarios, \gls{os}1 and \gls{os}2, represent the core situations encountered in railway operations, highlighting the control system's critical role in ensuring safety and efficiency. These scenarios underscore the system’s ability to manage the most fundamental and challenging aspects of railway operations, ensuring reliability in both routine and disrupted conditions.
		...
		In \gls{os}2, despite degraded communication, the follower's control system processes data faster than in \gls{os}1, demonstrating efficiency with a lower mean computation time and highlighting robustness under challenging conditions.
...
%				 First plot
%				\nextgroupplot[
%				title={\textbf{Safety barrier}},
%				xlabel={$t(\unit{\second})$},
%				ylabel={$\mathrm{distance}(\unit{\meter})$}, 
%				yticklabel style = {font=\footnotesize},
%				ymax = 200,
%				legend pos=south east,
%				yticklabel style={/pgf/number format/.cd, fixed, fixed zerofill, precision=0, /tikz/.cd},
%				                            ymin=0, ymax=40, % Custom y-axis limits
%				xticklabels=\empty,  % This removes only the x-axis labels but keeps the ticks
%				grid = both, 				
%				]
%				
%				\addplot [name path=greenCurve, color=green, solid, line width=1pt] 
...
%				 Second plot
%				\nextgroupplot[
%				title={\textbf{Interdistance}},
%				xlabel={$t(\unit{\second})$},
%				ylabel={$\mathrm{distance}(\unit{\meter})$}, 
%				legend pos=north east,
%				ymin=0, ymax=6200, % Custom y-axis limits
%				xticklabels=\empty,  % This removes only the x-axis labels but keeps the ticks
%				grid = both,
%				]
%				\addplot [name path=greenCurve, color=black, solid, line width=1pt] 
%				file{dataMatlab/OS1/interDistance.txt};
%					\addlegendentry{$x_1^{\mathrm{L}}(t)-x_1^{\mathrm{F}}(t)$}
%				
...
%				 Third plot
%				\nextgroupplot[
%				title={\textbf{Velocities}},
%				xlabel={$t(\unit{\second})$},
%				ylabel={$\mathrm{speed}(\frac{\unit{\meter}}{\unit{\second}})$}, 
%				xlabel={(4) },
%				legend pos=north east,
%				                    ymin=0, ymax=6200, % Custom y-axis limits
%				xticklabels=\empty,  % This removes only the x-axis labels but keeps the ticks
...
%				\addlegendentry{$x_2^{\mathrm{L}}(t)$}
%				
%				\addplot [name path=leader, color=brown, solid, line width=1pt] 
%				file{dataMatlab/OS1/velocityFollower.txt};
%				\addlegendentry{$x_2^{\mathrm{F}}(t)$}
%				
%				 Fourth plot
%				\nextgroupplot[
%				title={\textbf{Control inputs}},
%				ylabel={control force$(\unit{\newton})$}, 
%				ymin=-380000, ymax=380000, % Custom y-axis limits
%				legend pos=south east,
%				 every x tick scale label/.style={at={(rel axis cs:1,0)},anchor=south west,inner sep=1pt},
%				xticklabels=\empty,  % This removes only the x-axis labels but keeps the ticks
%				grid = both,
...
%				\addlegendentry{$u^{\mathrm{L}}(t)$}
%				
%				\addplot [name path=leader, color=brown, solid, line width=1pt] 
%				file{dataMatlab/OS1/forceFollower.txt};
%				\addlegendentry{$u^{\mathrm{F}}(t)$}
%				
%				 Fifth plot
%				\nextgroupplot[
%				title={\textbf{Follower controller}},
%				ylabel={controllers}, 
%				ymin=0.75,
%				ymax=4.25,
%				ymin=-380000, ymax=380000, % Custom y-axis limits
...
%				grid = both,
%				ytick={-1, 1, 2, 3, 4},
%				yticklabels={$K^{\mathrm{F}}_{\mathrm{L3}}$, $K^{\mathrm{F}}_1$, $K^{\mathrm{F}}_2$, $K^{\mathrm{F}}_3$, $K^{\mathrm{F}}_{\mathrm{E}}$},
%				]
%				
%				\addplot [name path=leader, color=orange, solid, line width=1pt] 
...
%				 Sixth plot
%				\nextgroupplot[
%				title={\textbf{Reference trajectories}},
...
%				legend pos=south east,
%				grid = both,
%				x tick label style={scaled ticks=base 10:-3}
%				y tick label style={scaled ticks=base 10:-3},
%				]
%				
%				\addplot [name path=leader, color=green, solid, line width=1pt] 
%				file{dataMatlab/OS1/z_tau_1.txt};
%				\addlegendentry{$ \mathbf{z}_{\tau_3}^\mathrm{F}(t)$}
%				
%				\addplot [name path=leader, color=orange, solid, line width=1pt] 
%				file{dataMatlab/OS1/z_tau_2.txt};
%				\addlegendentry{$ \mathbf{z}_{\tau_2}^\mathrm{F}(t)$}
%				
%				\addplot [name path=leader, color=red, solid, line width=1pt] 
%				file{dataMatlab/OS1/z_tau_3.txt};
%				\addlegendentry{$ \mathbf{z}_{\tau_1}^\mathrm{F}(t)$}
%				
%				\addplot [name path=leader, color=black, solid, line width=1pt] 
%				file{dataMatlab/OS1/distanceFollower.txt};
%				\addlegendentry{$x_1^{\mathrm{F}}(t)$}
%				
%				
%			\end{groupplot}
%		\end{tikzpicture}
%		\caption{Simulation results for \gls{os}1.}
%		\label{fig:os1}
%	\end{minipage}
%\end{figure*}
...
\begin{subfigure}{0.5\linewidth}
	...
	\begin{tikzpicture}[trim axis left]
			\begin{axis}[
					...
					ylabel={Controllers},
					ytick={  3, 4},
					yticklabels={$K^{\mathrm{F}}_3$, $K^{\mathrm{F}}_{\mathrm{E}}$}
					]
					\addplot [name path=leader, color=orange, solid, line width=1pt] 
...
\end{subfigure}
	...
	\caption{Plots for operational scenario OS1. Nel cuore della città, tra strade affollate e suoni incessanti, un piccolo caffè si nascondeva tra edifici imponenti. Ogni mattina, l'aroma del caffè appena macinato si diffondeva nell'aria, attirando passanti stanchi e sognatori. Marta, seduta al solito tavolo accanto alla finestra, osservava la vita scorrere veloce fuori. Sognava di viaggi lontani, di cieli infiniti e avventure ancora da vivere. Il tempo sembrava fermarsi in quel luogo, mentre il mondo continuava a girare. Ogni tazza di caffè era un nuovo inizio, un momento di riflessione silenziosa. imponenti. Ogni mattina, l'aroma del caffè appena macinato si diffondeva nell'aria, attirando passanti stanchi e sognatori. Marta, seduta al solito tavolo accanto alla finestra, osservava la vita scorrere veloce fuori. Sognava di viaggi lontani, di cieli infiniti e avventure ancora da vivere. Il tempo sembrava fermarsi in quel luogo, mentre il mondo continuava a girare. Ogni tazza di caffè era un nuovo inizio, un momento di riflessione silenziosa.}
...
%					only marks,
%					mark size=0.5pt
%					]file{dataMatlab/OS2/event_z_region.txt};
%				\end{axis}
%			\end{tikzpicture}
%			\caption{}
%			\label{fig:os2_e}
...
%					]
%					\addplot [name path=leader, color=green, solid, line width=1pt] 
%					file{dataMatlab/OS2/z_tau_1.txt};
%					\addlegendentry{$ \mathbf{z}_{\tau_3}^\mathrm{F}(t)$}
%					
%					\addplot [name path=leader, color=orange, solid, line width=1pt] 
%					file{dataMatlab/OS2/z_tau_2.txt};
%					\addlegendentry{$ \mathbf{z}_{\tau_2}^\mathrm{F}(t)$}
%					
%					\addplot [name path=leader, color=red, solid, line width=1pt] 
%					file{dataMatlab/OS2/z_tau_3.txt};
%					\addlegendentry{$ \mathbf{z}_{\tau_1}^\mathrm{F}(t)$}
%					
%					\addplot [name path=leader, color=black, solid, line width=1pt] 
%					file{dataMatlab/OS2/distanceFollower.txt};
%					\addlegendentry{$x_1^{\mathrm{F}}(t)$}
%				\end{axis}
%			\end{tikzpicture}
%			\caption{}
...
%		\end{subfigure}%
%		
%		\caption{Plots for operational scenario OS1.}
%
%\end{figure*}
...
%				% Sixth plot
%				\nextgroupplot[
%				title={\textbf{Delay communication channel}},
%				legend style={font=\small},
%				xlabel={$t(\unit{\second})$},
...
%			\end{groupplot}
%		\end{tikzpicture}
%		\caption{Simulation results for \gls{os}2.}
%		\label{fig:os2}
%	\end{minipage}
%\end{figure*}
%
