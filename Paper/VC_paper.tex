...
\blue{Aggiungere $\hat{\tau}$ nell'algoritmo che proviene dal blocco di stimatore del ritardo. Dove dobbiamo metterlo? sia nell'algoritmo e sia nel testo, giusto?}
...
The controller $K^{\mathrm{F}}_2(x^{\mathrm{F}})$ is tasked with supporting the \gls{vc} under normal operating conditions. Its primary objective is to maintain the average velocity of the leader, ensuring smooth and efficient operation. This controller works to minimize deviations from the desired velocity and position, while also optimizing control effort. Its functional cost is defined as follows
%
%
\begin{equation}
	\label{eq:K_2_controller}
	\begin{aligned}
		K^{\mathrm{F}}_2(x^\mathrm{F}) = \arg  \min_{u_(t)} \int_{t}^{t+H}& \Theta^s_2(x_1^{\mathrm{F}}(r)-\mathbf{z}_{\tau_2}^\mathrm{F}(t_{\mathrm{last}})[r])^2 \\ &+\Theta^v_2(x_2^{\mathrm{F}}(r)-\underline{\mathbf{x}}_2^\mathrm{L}(r))^2  +  \Theta^u_2(u^{\mathrm{F}}(r))^2 dr, \\
...
%  					height=3cm,
  					...
  					ylabel={\blue{slope ($\frac{\unit{\milli\meter}}{\unit{\meter}}$)}},
  					yticklabel style={/pgf/number format/fixed},
  					yticklabel style={color=blue},
  					title={Railway line profile},
  					axis y line*=left,
  					axis x line*=bottom,
  					ymin=-19, ymax=19, % Custom y-axis limits
  					grid=both
  					]
...
%  					height=3cm,
  					xlabel={s (m)},
  					ylabel={\red{curvature (\unit{\meter})}},
  					...
  					\addplot [const plot, name path=greenCurve, color=red, dashed, line width=1pt] 
...
%							ylabel={$\mathrm{Seconds}[s]$},
%							ymin=0, ymax=40, % Custom y-axis limits
%							grid = both
%							]
%							
%							\addplot [name path=greenCurve, color=green, solid, line width=0.5pt] 
%							file{dataMatlab/OS1/tau1.txt};
%							\addlegendentry{$\tau_1$}
%							
%							\addplot [name path=yellowCurve,color=orange, solid, line width=0.5pt] 
%							file{dataMatlab/OS1/tau2.txt};
%							\addlegendentry{$\tau_2$}
%							
%							\addplot [name path=redCurve,color=red, solid, line width=0.5pt] 
%							file{dataMatlab/OS1/tau3.txt};
%							\addlegendentry{$\tau_3$}
%							
%							\addplot [color=blue, solid, line width=0.15pt] 
%							file{dataMatlab/OS1/tp.txt};%
%							\addlegendentry{$t_p$}
%							
%							
%							
%							\coordinate (spypoint) at (axis cs:500,2);
%							\coordinate (magnifyglass) at (axis cs:500,20);
%							
%							
%						\end{axis}
%					
...
%				\end{tikzpicture}
%			\caption{Subtemporal divisions $\tau_1, \tau_2, \tau_3$ and time passed from the last received data packet $t_p$.}
%			\label{fig:os1Tau}
%		\end{figure}
...
%				\draw[red, solid, line width=2pt] (axis cs:0,0) -- (axis cs:1000,0);
%				
%				\legend{$b(x)$}
%				
%				
...
%				ymin=45, ymax=85, % Custom y-axis limits
%				legend pos=north east,
%				%                    ymin=0, ymax=6200, % Custom y-axis limits
%				grid = both
%				]
...
%				
%				\addplot [name path=leader, color=orange, solid, line width=1pt] 
%				file{dataMatlab/OS1/Ztau_1.txt};
%				
%				
%				% Sixth plot
%				\nextgroupplot[
...
%				ylabel={$\mathrm{distance}(\unit{\meter})$}, 
%				%yticklabel style = {font=\footnotesize},
%				y tick label style={scaled ticks=base 10:-2},
%				ymin = -400,
%				ymax = 100,
%				legend pos=south east,
%				yticklabel style={/pgf/number format/.cd, fixed, fixed zerofill, precision=0, /tikz/.cd},
...
%				ylabel={$\mathrm{distance}(\unit{\meter})$}, 
%				legend pos=south east,
%				ymin=500, ymax=2000, % Custom y-axis limits
%				grid = both,
%				y tick label style={scaled ticks=base 10:-3}
%				]
%				\addplot [name path=greenCurve, color=black, solid, line width=1pt] 
...
%			%	xlabel={$t(\unit{\second})$},
%				ylabel={$\mathrm{speed}(\frac{\unit{\meter}}{\unit{\second}})$}, 
%				ymin=-10, ymax=85, % Custom y-axis limits
%				legend pos=north east,
%				%                        ymin=0, ymax=6200, % Custom y-axis limits
%				grid = both
%				]
...
%				
%				\addplot [name path=leader, color=orange, solid, line width=1pt] 
%				file{dataMatlab/OS2/Ztau_1.txt};
%				
%				
%				% Sixth plot
%				\nextgroupplot[
...
%				height= 6 cm,
%				xlabel={$t(\unit{\second})$},
%				ylabel={reference trajectories$(\unit{\meter})$}, 
%				%ymin=-380000, ymax=380000, % Custom y-axis limits
%				legend pos=south east,
%				%                            ymin=0, ymax=40, % Custom y-axis limits
%				grid = both,
...
%				
%				\addplot [name path=leader, color=green, solid, line width=1pt] 
%				file{dataMatlab/OS2/tau1_1.txt};
%				\addlegendentry{$ \mathbf{z}_{\tau_3}^\mathrm{F}(t)$}
%				
%				\addplot [name path=leader, color=orange, solid, line width=1pt] 
%				file{dataMatlab/OS2/tau2_1.txt};
%				\addlegendentry{$ \mathbf{z}_{\tau_2}^\mathrm{F}(t)$}
%				
%				\addplot [name path=leader, color=red, solid, line width=1pt] 
%				file{dataMatlab/OS2/tau3_1.txt};
%				\addlegendentry{$ \mathbf{z}_{\tau_1}^\mathrm{F}(t)$}
%				
%				\addplot [name path=leader, color=black, solid, line width=1pt] 
%				file{dataMatlab/OS2/distanceFollower_1.txt};
%				\addlegendentry{$x_1^{\mathrm{F}}(t)$}
%				
%				%\addlegendentry{$u^{\mathrm{L}}(t)$}
%			\end{groupplot}
...
%			ymin=0, ymax=4200, % Custom y-axis limits
%			grid = both,
%			y tick label style={scaled ticks=base 10:-3}
%			]
%			\addplot [name path=greenCurve, color=black, solid, line width=1pt] 
%			file{dataMatlab/OS3/interDistance.txt};
%			\addlegendentry{$x_1^{\mathrm{L}}(t)-x_1^{\mathrm{F}}(t)$}
%			
%			\addplot [name path=greenCurve, color=red, solid, line width=1.5pt] 
%			file{dataMatlab/d3.txt};
%			
%			\addlegendentry{$d$}
%			
%			% Third plot
...
%			%xlabel={$t(\unit{\second})$},
%			ylabel={$\mathrm{speed}(\frac{\unit{\meter}}{\unit{\second}})$}, 
%			ymin=30, ymax=80, % Custom y-axis limits
%			legend pos=south west,
%			%						ymin=0, ymax=6200, % Custom y-axis limits
%			grid = both
%			]
...
%			
%			\addplot [name path=leader, color=orange, solid, line width=1pt] 
%			file{dataMatlab/OS3/velocityLeader.txt};
%			\addlegendentry{$x_2^{\mathrm{L}}(t)$}
%			
%			\addplot [name path=leader, color=brown, solid, line width=1pt] 
%			file{dataMatlab/OS3/velocityFollower.txt};
%			\addlegendentry{$x_2^{\mathrm{F}}(t)$}
%			
%			% Fourth plot
%			\nextgroupplot[
%			title={\textbf{Estimated delay}},
%			height= 6 cm,
%			xlabel={$t(\unit{\second})$},
%			ylabel={delay$(\unit{\second})$}, 
%			%ymin=-380000, ymax=380000, % Custom y-axis limits
%			legend pos= south east,
%			%							ymin=0, ymax=40, % Custom y-axis limits
%			grid = both,
...
%			
%			\addplot [name path=leader, color=orange, solid, line width=1pt] 
%			file{dataMatlab/OS3/tauEstimated_1.txt};
%			\addlegendentry{$\hat{\tau}(t)$}
%			
%			
%			% Fifth plot
%			\nextgroupplot[
...
%			
%			\addplot [name path=leader, color=orange, solid, line width=1pt] 
%			file{dataMatlab/OS3/Ztau_1.txt};
%			
%			
%			% Sixth plot
%			\nextgroupplot[
...
%			
%			\addplot [name path=leader, color=green, solid, line width=1pt] 
%			file{dataMatlab/OS3/tau1_1.txt};
%			\addlegendentry{$ \mathbf{z}_{\tau_3}^\mathrm{F}(t)$}
%			
%			\addplot [name path=leader, color=orange, solid, line width=1pt] 
%			file{dataMatlab/OS3/tau2_1.txt};
%			\addlegendentry{$ \mathbf{z}_{\tau_2}^\mathrm{F}(t)$}
%			
%			\addplot [name path=leader, color=red, solid, line width=1pt] 
%			file{dataMatlab/OS3/tau3_1.txt};
%			\addlegendentry{$ \mathbf{z}_{\tau_1}^\mathrm{F}(t)$}
%			
%			\addplot [name path=leader, color=black, solid, line width=1pt] 
%			file{dataMatlab/OS3/distanceFollower_1.txt};
%			\addlegendentry{$x_1^{\mathrm{F}}(t)$}
%			
%			
%			
...
%		\end{groupplot}
%	\end{tikzpicture}
%	\caption{Simulation results for \gls{os}3.}
%	\label{fig:os3}
%\end{figure*}
