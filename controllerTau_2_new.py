...
for t in range(0, nu*N-1, nu):
    ...
    # if t>0:
        # cost +=  r_past*cs.dot((u_t-u_t_past), (u_t-u_t_past))
    cost += (qs*(s-z0[t+4]))**2 +  (qv*(v-v_target))**2 + (r*u_t)**2 + ((r_past*(u_t-u_t_past))**2)
...
fmin = [-0.000001]*(fu_P.size1()) + [0]*(fv.size1()) 
fmax = [0.000001]*(fu_P.size1()) + [83]*(fv.size1()) 
...
meta = og.config.OptimizerMeta()\
    .with_optimizer_name("controller_tau_2_new")
