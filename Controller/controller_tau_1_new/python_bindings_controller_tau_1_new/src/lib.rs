///
/// Auto-generated python bindings for optimizer: controller_tau_1_new
///
...
use controller_tau_1_new::*;

#[pymodule]
fn controller_tau_1_new(_py: Python, m: &PyModule) -> PyResult<()> {
...
impl Solver {
    ...
    fn run(
        ...
        let mut u = [0.0; CONTROLLER_TAU_1_NEW_NUM_DECISION_VARIABLES];

        // ----------------------------------------------------
        // Set initial value
        // ----------------------------------------------------
        if let Some(u0) = initial_guess {
            if u0.len() != CONTROLLER_TAU_1_NEW_NUM_DECISION_VARIABLES {
                println!(
                    "1600 -> Initial guess has incompatible dimensions: {} != {}",
                    u0.len(),
                    CONTROLLER_TAU_1_NEW_NUM_DECISION_VARIABLES
                );
                return Ok(None);
            }
        ...
        if let Some(y0) = &initial_lagrange_multipliers {
            if y0.len() != CONTROLLER_TAU_1_NEW_N1 {
                println!(
        ...
        // Check parameter
        // ----------------------------------------------------
        if p.len() != CONTROLLER_TAU_1_NEW_NUM_PARAMETERS {
            println!(
                "3003 -> wrong number of parameters: {} != {}",
                p.len(),
                CONTROLLER_TAU_1_NEW_NUM_PARAMETERS
            );
            return Ok(None);
        }
